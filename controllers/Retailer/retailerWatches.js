...
module.exports.UpdateRetailerWatchDiscount = async function(req, res, next)
{
    try
    {
        ...
        const setRetailerWatchDiscount = async function(reason)
        {
            ...
            let message;
            if(reason)
                message = 'Successfully updated your retailer discount on watch with reference number: ' + watch.referenceNumber + ' to be ' + existingWatch.retailerWatchDiscount + '% as it is less than or equal to the ' + reason;
            else
                message = 'Successfully updated your retailer discount on watch with reference number: ' + watch.referenceNumber + ' to be ' + existingWatch.retailerWatchDiscount + '%';
        ...
        if(retailerMaximumWatchDiscount)
        {
            if(retailerMaximumWatchDiscount.maximumWatchDiscount >= retailerWatchDiscount)
                return await setRetailerWatchDiscount('maximum discount allowed on this watch allowed for your retailer account which is ' + retailerMaximumWatchDiscount.maximumWatchDiscount + '%');
            else
        ...
        if(retailerMaximumCollectionDiscount)
        {
            if(retailerMaximumCollectionDiscount.maximumCollectionDiscount >= retailerWatchDiscount)
                return await setRetailerWatchDiscount('maximum discount allowed on this collection for your retailer account which is ' + retailerMaximumCollectionDiscount.maximumCollectionDiscount + '%');
            else
        ...
        if(retailerMaximumBrandDiscount)
        {
            if(retailerMaximumBrandDiscount.maximumBrandDiscount >= retailerWatchDiscount)
                return await setRetailerWatchDiscount('maximum discount allowed on this brand for your retailer account which is ' + retailerMaximumBrandDiscount.maximumBrandDiscount + '%');
            else
        ...
        if(watch.maximumDiscount)
        {
            if(watch.maximumDiscount >= retailerWatchDiscount)
                return await setRetailerWatchDiscount('maximum discount allowed on this watch which is ' + watch.maximumDiscount + '%');
            else
        ...
        if(watch.collectionObject.maximumDiscount)
        {
            if(watch.collectionObject.maximumDiscount >= retailerWatchDiscount)
                return await setRetailerWatchDiscount('maximum discount allowed on this collection which is ' + watch.collectionObject.maximumDiscount + '%');
            else
        ...
        if(watch.brandObject.maximumDiscount)
        {
            if(watch.brandObject.maximumDiscount >= retailerWatchDiscount)
                return await setRetailerWatchDiscount('maximum discount allowed on this brand which is ' + watch.brandObject.maximumDiscount + '%');
            else
